...
```{r}
datapasta::df_paste(head(mack_creek_vertebrates, 5) [, c("SPECIES", "SECTION", "YEAR", "SAMPLEDATE", "LENGTH1")])
...
mack_creek_lengths <- mack_creek_vertebrates %>% 
  ...
  select(year:sampledate) %>% 
  filter(section == "CC") %>% 
  mutate(across(where(is.character, tolower))) %>% data.frame(
  stringsAsFactors = FALSE,
...
```{r}
datapasta::df_paste(head(mack_creek_vertebrates, 5) [, c("SPECIES", "WEIGHT", "LENGTH1")])
